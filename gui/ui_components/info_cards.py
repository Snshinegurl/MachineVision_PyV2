...
def create_file_info_cards():
    widget = QWidget()
    # Use horizontal layout to keep all cards in one line
    ...
    # Add each card with stretch so they expand evenly
    for card in (file_name_card, file_size_card, dimensions_card, pixels_card, format_card, object_area_card):
        layout.addWidget(card, 1)  # stretch factor 1

    cards = (file_name_card, file_size_card, dimensions_card, pixels_card, format_card, object_area_card)
    return widget, cards
...
def create_info_card(label, value, color, icon_name):
    ...
    widget.setMinimumWidth(150)   # ensure minimum width to prevent excessive shrinking
    ...
    icon_map = {
        ...
        'cube': 'fa5s.cube'  # for object area
    }
...
def update_info_cards(image_info, file_name_card, file_size_card, dimensions_card, pixels_card, format_card, object_area_card):
    ...
    # For object area, initially set to total pixels (or later update after background removal)
    object_area_card.findChild(QLabel, "card-value").setText(f"{pixels:,} (total)")